...
如果 `rg` 找不到，说明这个 PC 可能不在 ELF 的 `.text` 中，常见原因是：
...
```bash
readelf -S build/load-store-riscv32-nemu.elf
readelf -h build/load-store-riscv32-nemu.elf
```

确认入口地址和 `.text` 地址范围是否在 `0x80000000` 附近。

### 场景 2：itrace 和 `.txt` 对不上

判断顺序：

1. `.txt` 是不是本次构建生成的。
2. NEMU 加载的是不是同一个 `.bin`。
3. ELF 链接地址是否是 `0x80000000`。
4. `objcopy` 后的 bin 是否被 `insert-arg` 修改过。

命令：

```bash
ls -l build/*riscv32-nemu.*
make ARCH=riscv32-nemu ALL=dummy clean
make ARCH=riscv32-nemu ALL=dummy run-batch
```

如果清理后恢复，说明之前是旧构建产物误导。
...
```bash
readelf -s build/dummy-riscv32-nemu.elf | less
objdump -d build/dummy-riscv32-nemu.elf | less
```

ftrace 也依赖 ELF。如果 ftrace 只有地址，先检查运行命令里是否传了：
...
```bash
xxd -g 4 -l 64 build/dummy-riscv32-nemu.bin
```

再和 `.txt` 开头机器码对比。RISC-V 小端存储，`objdump` 显示的 32 位机器码在 bin
中会按低字节在前排列。
